...
def main(cfg_path: str):
    ...
    print(f"[Info] Current Using: {processed_filelist}")
    ...
    print(f"[Info] Train/Vali/Test = {len(train_ds)} / {len(val_ds)} / {len(test_ds)}")

    train_loader = DataLoader(train_ds, batch_size=cfg["batch_size"], shuffle=True, num_workers=cfg.get("num_workers", 0), pin_memory=True if cfg.get("num_workers", 0) > 0 else False)
    ...
    model = SwinUnetLV(img_size=cfg["image_size"], pretrained=cfg.get("pretrained", False)).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    criterion = CombinedLoss(bce_weight=0.0, dice_weight=1.0)
    scaler = torch.amp.GradScaler('cuda', enabled=cfg.get("amp", True))
    main_scheduler = CosineAnnealingLR(optimizer, T_max=(cfg["num_epochs"] - warmup_epochs) * len(train_loader), eta_min=1e-6)
    ...
    for epoch in range(cfg["num_epochs"]):
        ...
        model.eval()
        val_running_loss, val_total_dice, val_total_iou = 0.0, 0.0, 0.0
        val_valid_batches = 0
        with torch.no_grad():
        ...
        print(f"Epoch {epoch}: Val Loss {val_loss_avg:.4f} | Dice: {val_dice_avg:.4f} | IoU: {val_iou_avg:.4f}")
        ...
        if val_dice_avg > best_val_dice:
            ...
            print(f"[✓] Saved best model to {ckpt_path} (Val Dice: {best_val_dice:.4f})")
    ...
    if best_ckpt_path is not None:
        ...
        print(f"[Info] Successfully load best model: {best_ckpt_path}")
        ...
        print("[Warning] Didn't find any model, skip test")
    print("="*50)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="configs/default.yaml", help="configs path")
